...
mono micro-labels. References live next door — read them when needed, don't reproduce them inline.
...
2. **Pick a layout pattern** — match the content shape to one of the patterns documented in
   [`references/patterns.md`](references/patterns.md). Read that file before writing if you're not
   sure what shape fits. Each pattern points at one or more of the 20 reference HTML files in
   `~/3rd/html-effectiveness/` (see [`references/gallery.md`](references/gallery.md)) that you can
   open and study for concrete structure.

3. **Write the HTML as a single self-contained file**. All CSS in a `<style>` block in `<head>`.
   ...
   pre-wired skeleton you can start from if you want a faster path.

4. **Save and open**. Write to `/tmp/<slug>.html`, then run `open /tmp/<slug>.html`. On macOS this
   opens the user's default browser. Do this in one step — don't ask "should I open it now?".

5. **Reply in chat with one short line**, e.g. "Opened `/tmp/pinia-vs-zustand.html` — compares the
...
- **Information density matters**. The reference pages pack a lot per screen — 4-column summary
  bands, multi-column card grids, tables with hover states. Don't pad with one-card-per-row layouts
  unless the content is genuinely one item.

## Content fits layout, not the other way around

The most common failure mode is taking the wrong content into the wrong layout — putting a 5-step
plan into a card grid (loses the sequence), or putting a comparison of 3 things into a long article
(makes the reader hunt for the table). Spend a moment matching shape to content:
...
More combinations in [`references/patterns.md`](references/patterns.md).
...
These are the patterns that make HTML output look AI-generated and "default" rather than crafted.
The reference gallery does none of these.
...
- [ ] `:root` design tokens present, palette matches the gallery
