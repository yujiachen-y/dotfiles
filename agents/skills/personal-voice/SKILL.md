...
name: personal-voice
description: >
  把 AI 风格的草稿改写成 yujiachen 的个人写作风格——克制的 emoji、紧凑短段落、精确的概率词（"往往""经常"而非"必然""直接"）、中性反差陈述（不打脸不唱衰）、英文术语空格分隔、第一人称收尾。
  当用户给出一段 AI 写的稿子并要求"按我的风格改"、"去 AI 味"、"润色一下"、"读起来自然点"、"听着不像 AI"、"改改这篇"、"减点 AI 味"、"小红书风"、"newsletter 风" 时主动使用本 skill。
  即使用户只说"帮我改改"或"看看怎么调整一下" — 只要上下文是结构化的中文 / 中英混用稿件（小红书 / newsletter / 工程博客 / 长文评论），都应当考虑触发，因为去 AI 味是 yujiachen 提出"改写"时的默认意图。
---

# Personal Voice Rewrite

把 AI 写的稿子改成 yujiachen 的写作风格。AI 输出有几个稳定的"坏味道"——堆装饰性 emoji、滥用情绪词（打脸 / 神话破灭 / 卷错）、机械分层（第一层 / 第二层）、绝对化副词（直接 / 必然 / 完全）、过渡句铺陈（接下来 / 综上 / 让我们）。yujiachen 的真实写作不是这样。

## 第一步：先读 test cases

`test-cases/` 目录下每个文件夹都是一个对照样本：
- `ai-draft.md`：典型的 AI 写法初稿
- `final.md`：yujiachen 改完的最终稿
- `README.md`（可选）：场景说明

**改写之前先随机抽 1-2 个 test case 通读 ai-draft 和 final**——你会快速看到一个稳定的差异 pattern：AI 在哪些地方"过度修饰"，yujiachen 把它们删到了哪里。这比读规则更直观。

如果用户给的稿子和某个 test case 场景接近（例如同样是小红书科普），优先看那一类。
...
- 一句话判断："这个 emoji 是装饰还是反应？" 装饰就删，反应就留
...
1. **读 test-cases**（至少 1 个）建立 calibration
2. **读用户给的 AI 草稿**，标记出违反上述规则的地方（emoji 装饰、情绪词、绝对化、机械分层、过渡句、AI 对比句式、悬空抽象词、自我评论、文学化收束、meta 开场、hashtag）
3. **改写**：每条规则都对照过一遍
4. **回看**：自己读一遍输出，问「这段读起来像 AI 还是像 yujiachen？」如果还有 AI 味，再改
5. **不要喧宾夺主**：如果用户的稿子已经写得不错，只改有问题的地方，不要全面重写
